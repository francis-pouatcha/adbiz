...
angular.module('AdBase').factory('connectionHistorysResource',['$http', function($http){
    var service = {};
    var urlBase = '/adbase.server/rest/connectionhistorys',
    searchInput = {
        entity:{},
        start:0,
        max:100,
    ...
    service.searchConnectionHistorys = function(searchInput) {
        return $http.post(urlBase+'/searchConnectionHistorys',searchInput);
    };

    service.findByLogin = function(entityId){
        return $http.get(urlBase+'/findByLogin/'+entityId);
    };

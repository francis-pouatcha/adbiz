...
.factory('genericResource',['$http','$q', function($http,$q, urlBase){
    ...
    service.findByLikePromissed = function (urlBase, fieldName, fieldValue, klassName){
        ...
        var coreSearchInput = {entity:{},fieldNames:[],start:0,max:10};
        coreSearchInput.entity[fieldName]= fieldValue;
    ...
    service.findByLikeWithSearchInputPromissed = function (urlBase, coreSearchInput){
    	if(angular.isUndefined(urlBase) || !urlBase ||
    		angular.isUndefined(coreSearchInput) || !coreSearchInput) return;          
          var deferred = $q.defer();
          service.findByLike(urlBase, coreSearchInput)
          .success(function(entitySearchResult) {
              deferred.resolve(entitySearchResult);
    ...
    service.findCustomWithSearchInputPromissed = function (urlBase, coreSearchInput){
    	if(angular.isUndefined(urlBase) || !urlBase ||
    		angular.isUndefined(coreSearchInput) || !coreSearchInput) return;          
          var deferred = $q.defer();
          service.findCustom(urlBase, coreSearchInput)
...
// displayInfoVar = {};
    		...
    		angular.copy(searchResultIn.coreSearchInput, searchResultVar.coreSearchInput);
        ...
        this.maxResult = function(maxResultIn) {
            if(adUtils.greaterThan(maxResultIn,-1)) searchResultVar.coreSearchInput.max=maxResultIn;
            return searchResultVar.coreSearchInput.max;
        };
        ...
        this.coreSearchInput = function(coreSearchInputIn, className){
            if(className){
                searchResultVar.coreSearchInput.className = className;
                return searchResultVar.coreSearchInput;
            }
            if(angular.isUndefined(coreSearchInputIn))
                return angular.copy(searchResultVar.coreSearchInput);

    		angular.copy(coreSearchInputIn, searchResultVar.coreSearchInput);
            return coreSearchInputIn;
        };
        this.coreSearchInputChanged = function(coreSearchInputIn){
            return angular.equals(searchResultVar.coreSearchInput, coreSearchInputIn);
        };
        this.newSearchInput = function(){
            return angular.copy(nakedSearchResult.coreSearchInput);
        };
        this.paginate = function(){
        	searchResultVar.coreSearchInput.start = ((searchResultVar.currentPage - 1)  * searchResultVar.itemPerPage);
        	searchResultVar.coreSearchInput.max = searchResultVar.itemPerPage;
            return handler.coreSearchInput();
        };

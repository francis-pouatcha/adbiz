...
# cp $ADCOM_HOME/adcom.configuration/jboss-eap-6.4/standalone/security/adcom.jks $JBOSS_HOME/standalone/security/adcom.jks


echo 'starting jboss'
cd $JBOSS_HOME
cd $JBOSS_HOME && rm standalone/log/*.log
cd $JBOSS_HOME && rm standalone/log/*.log.*

cd $JBOSS_HOME && bin/standalone.sh --debug >/dev/null &

(function () {
    ...
    var appModule = angular.module('adbnsptnr', [
        'app.core',
        'app.layout'
    ]);
    ...
    angular.element(document).ready(function ($http, Auth) {
        ...
        keycloakAuth.init({onLoad: 'login-required'}).success(function () {
            ...
            auth.logoutUrl = keycloakAuth.authServerUrl +
                '/realms/adcom/tokens/logout?redirect_uri=/adbnsptnr.client';
            //Auth = auth;
            appModule.factory('Auth', function() {
                return auth;
            });
            angular.bootstrap(document, ['adbnsptnr']);
        }).error(function () {
            console.log(keycloakAuth);
    ...
    appModule.controller('GlobalController', function($rootScope, Auth, BASE_ROUTE, BASE_SERVER) {
        ...
        $rootScope.appTitle = 'Business Partner';
    });
    /* jshint ignore:end */

 ...
 * @author fpo
 */
public class B2ConnectionProviderFactory implements JpaConnectionProviderFactory, ServerInfoAwareProviderFactory {

    private static final Logger logger = Logger.getLogger(B2ConnectionProviderFactory.class);
    ...
    @Override
    public B2ConnectionProvider create(KeycloakSession session) {
        lazyInit(session);

        EntityManager em = emf.createEntityManager();
        em = PersistenceExceptionConverter.create(em);
        session.getTransaction().enlist(new JpaKeycloakTransaction(em));
        return new B2ConnectionProvider(em);
    }
...
//	
//	                        B2UpdaterProvider updater = session.getProvider(B2UpdaterProvider.class);
//	                        if (updater == null) {
...
//	                            }
//	
//	                            if (currentVersion == null || !B2UpdaterProvider.LAST_VERSION.equals(currentVersion)) {
